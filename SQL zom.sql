UPDATE zdata SET Datekey_Opening = REPLACE(Datekey_Opening, '_', '/') WHERE Datekey_Opening LIKE '%_%';
alter table zdata modify column Datekey_Opening date;
select * from zdata;
-- Percentage of Restaurants based on Has_Online_delivery
SELECT 
    has_online_delivery,
    CONCAT(ROUND(COUNT(Has_Online_delivery) / (SELECT COUNT(*) FROM zdata) * 100, 1), '%') AS percentage 
FROM 
    zdata 
GROUP BY 
    has_online_delivery;
...
SELECT  
    sheet2.country_name, 
    zdata.city, 
    COUNT(zdata.restaurantname) AS no_of_restaurants
FROM  
    zdata
INNER JOIN  
    sheet2 ON zdata.countrycode = sheet2.countryid
GROUP BY  
    sheet2.country_name, 
    zdata.city
ORDER BY  
...
from zdata group by year(datekey_opening),quarter(datekey_opening),monthname(datekey_opening) 
...
select has_table_booking,concat(round(count(has_table_booking)/100,1),"%") percentage
 from zdata group by has_table_booking;
 
 -- # highest rating restaurants in each country 
SELECT  
    sheet2.country_name, 
    zdata.restaurantname, 
    MAX(zdata.rating) AS highest_rating 
FROM  
    zdata 
INNER JOIN  
    sheet2 ON zdata.countrycode = sheet2.countryid 
...
SELECT  
    sheet2.country_name, 
    zdata.restaurantname, 
    MAX(zdata.votes) AS max_votes, 
    AVG(zdata.Average_Cost_for_two) AS avg_cost_for_two 
FROM  
    zdata 
INNER JOIN  
    sheet2 ON zdata.countrycode = sheet2.countryid 
